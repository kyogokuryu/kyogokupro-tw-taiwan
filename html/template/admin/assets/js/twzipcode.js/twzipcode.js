        ...
        init () {
            ...
            this.container.forEach((elem) => {
                ...
                if (Helper.isDOM(c, d)) {
                    ...
                    Object.keys(this.database).forEach((county,i) => {
                        ...
                        if (!(c1 || c2)) {
                            h.push(`<option value="${i+1}">${county}</option>`);
                        }
                    });
        ...
        bindEvents () {
            ...
            const onZipcodeKeyup = function (e) {
                ...
                if (Helper.isObject(find) && 'county' in find && 'district' in find && 'zipcode' in find) {
                    ...
                    let key = 0;
                    Object.keys(database).forEach((county,i) => {
                        if (county === find.county) {
                            key = i+1;
                        }
                    });
                    
                    wait().then(() => {
                        this.county.value = key;
                    ...
                    wait().then(() => {
                        this.zipcode.value = find.zipcode;
                        trigger(this.zipcode, 'change');
                        return wait();
                    });
