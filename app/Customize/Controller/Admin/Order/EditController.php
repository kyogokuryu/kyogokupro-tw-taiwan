...
class EditController extends AbstractController
{
    ...
    public function __construct(
        ...
        MailService $mailService//20220725 kikuzawa
    ) {
        ...
        $this->mailService = $mailService;//20220725 kikuzawa
    }
            ...
            switch ($request->get('mode')) {
                case 'register':
                    ...
                    if (!$flowResult->hasError() && $form->isValid()) {
                        ...
                        if ($OldStatus && $TargetOrder->getId() && $OldStatus->getId() != $NewStatus->getId()) {
                            ...
                            if($NewStatus->getId() == 6){
                                $this->checkGift($TargetOrder);
                            }
                        }
    ...
    public function searchProduct(Request $request, $page_no = null, Paginator $paginator)
    {
        if ($request->isXmlHttpRequest() && $this->isTokenValid()) {
            ...
            if ('POST' === $request->getMethod()) {
                ...
                $searchData['status'] = [1]; //公開されているものだけ表示
                $searchData['isExclusionTikTok'] = true;//$request->get('isExclusionTikTok');
