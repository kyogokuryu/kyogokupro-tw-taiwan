...
class Urank extends Command{
    ...
    protected static $defaultName = 'custom:urank';

    protected function configure()
    {
        $this->setDescription('Urank');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $offset = 0;
        while(true){
            $qb = $this->customerRepository->createQueryBuilder('o')
...
//print_r($Customer->getId());
            //    $Customer = $this->customerRepository->find($v->getId());
                $sale = (array)$this->getSalesByYear($Customer);
                if(count($sale)){
                    $order_amount = $sale["order_amount"] ? $sale["order_amount"] : 0;
                    $order_count = $sale["order_count"];

    //var_dump($sale->order_amount);
    //\Doctrine\Common\Util\Debug::dump((array)$sale, 3);
    //exit;
                    $Customer->setOwnerVal($order_amount);

                    if($order_amount >= 150000){
                        $Customer->setOwnerRank(3);
                    }elseif($order_amount >= 100000){
                        $Customer->setOwnerRank(2);
                    }elseif($order_amount >= 30000){
                        $Customer->setOwnerRank(1);
                    }else{
                        $Customer->setOwnerRank(0);
                    }
                }else{
                    $Customer->setOwnerVal(0);
                    $Customer->setOwnerRank(0);
                }
            ...
            echo "offset : " . $offset . " rcnt :" . $rcnt . PHP_EOL;
            if($rcnt > 0){
                $offset += $limit;
            }else{
                break;
