 ...
 */
class CustomizeCustomerRepository extends AbstractRepository
{
    /**
     * @var Queries
     */
    ...
    public function getQueryBuilderBySearchData($searchData)
    {
        ...
        // status
        if (!empty($searchData['prime_member']) && count($searchData['prime_member']) > 0) {
            $qb
                ->andWhere($qb->expr()->in('c.prime_member', ':prime_members'))
                ->setParameter('prime_members', $searchData['prime_member']);
        }
        ...
        // salon_id 子がいる会員を抽出(salon_idに自身の会員idを指定)して子の売上をセット 20220623 kikuzawa
        if (!empty($searchData['salonaf'])) {

            //子の売上の取得期間をセット
            $payment_date_start = '';
            $payment_date_end = '';
            if($searchData['payment_date_start'] && $searchData['payment_date_end']){
                $payment_date_start = $searchData['payment_date_start'];
                $payment_date_end = clone $searchData['payment_date_end'];
            }
            else{
                $payment_date_start = new \DateTime('first day of this month');
                $payment_date_start = new \DateTime($payment_date_start->format('Y-m-d'));
                $payment_date_end = clone $payment_date_start;
            }
            $payment_date_end->modify('+1 month');

            $qb
                ->addSelect('SUM(sr.sales) as sales')
                ->addSelect('SUM(sr.reward) as reward')
                ->leftJoin('\Customize\Entity\SalonafResult', 'sr', \Doctrine\ORM\Query\Expr\Join::WITH, 'c.id = sr.Customer')
                ->andWhere('sr.month >= :payment_date_start')
                ->andWhere('sr.month < :payment_date_end')
                ->setParameter('payment_date_start', $payment_date_start)
                ->setParameter('payment_date_end', $payment_date_end)
                ->groupBy('c.id');

            //未払い検索
            if(isset($_POST['mode']) && $_POST['mode'] == 'unpaid'){
                $qb
                    ->andWhere('sr.paid_flg = 0');
            }
        }
