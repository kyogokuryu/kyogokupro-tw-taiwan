...
namespace Plugin\uc_slide_cbd_sp;
...
class PluginManager extends AbstractPluginManager
{
     ...
     * @var string ブロック名
     */
    private $blockName = 'UCスライドショー CBD(スマホ)';

    /**
     * @var string ブロックファイル名
     */
    private $blockFileName = 'uc_slide_cbd_sp_block';
    ...
    {
        $Config = $em->find(uc_slide_cbd_spConfig::class, 1);
        if ($Config) {
            return $Config;
        }

        $Config = new uc_slide_cbd_spConfig();
        $Config->setBlockType(0);
        $Config->setSlideType(0);
        $em->persist($Config);
        $em->flush($Config);
