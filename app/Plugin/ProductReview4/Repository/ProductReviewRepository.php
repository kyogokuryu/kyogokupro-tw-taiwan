...
class ProductReviewRepository extends AbstractRepository
{
     ...
     * @return mixed
     */
    public function getStarAll(Product $Product)
    {
        $defaults = [
            ['recommend_level' => 1,'recommend_count' => 0,],
            ['recommend_level' => 2,'recommend_count' => 0,],
            ['recommend_level' => 3,'recommend_count' => 0,],
            ['recommend_level' => 4,'recommend_count' => 0,],
            ['recommend_level' => 5,'recommend_count' => 0,],
        ];
        try {
            $qb = $this->createQueryBuilder('r')
                ->select('r.recommend_level, count(r.id) as recommend_count')
                ->leftJoin('r.Product', 'p')
                ->where('r.Product = :Product')
                ->setParameter('Product', $Product)
                ->andWhere('r.Status = 1')
                ->groupBy('r.recommend_level');
