...
class RegularOrderRepository extends AbstractRepository
{
    ...
    public function getAllPrimeCancelStatusProducts($Config){
        ...
        //$qb->select('o.custome_id, max(regular_order_id) as regular_order_id')
        //    ->from(Re)
        $deadLineStartDate = new DateTime('today');
        $deadLineStartDate->modify('-30 day');
        $deadLineEndDate = new DateTime('tomorrow');
        $deadLineEndDate->modify('+'.$Config->getRegularOrderDeadline().' day');

        $qb = $this->createQueryBuilder('ro');
        $qb
            ->select('IDENTITY(ro.Customer), max(ro.id)')
            ->innerJoin('ro.RegularOrderItems', 'roi')
            //->innerJoin('ro.RegularShippings', 'rs')
            ->innerJoin('ro.Customer','c')            
            ->where(
                $qb->expr()->in('ro.RegularStatus', ':regularStatusIds')
            )->andWhere(
                $qb->expr()->in('c.prime_member', ':prime_member')
            )
        //    ->andWhere(
        //        $qb->expr()->gte('rs.next_delivery_date', ':deadLineDateStart')
        //    )
        ...
        //        $qb->expr()->lt('rs.next_delivery_date', ':deadLineDateEnd')
        //    )
            ->andWhere($qb->expr()->in('roi.Product',':products'))
            ->setParameter('regularStatusIds', [RegularStatus::CANCELLATION])
        //    ->setParameter('deadLineDateStart', $deadLineStartDate)
        //    ->setParameter('deadLineDateEnd', $deadLineEndDate)
            ->setParameter('products', Product::get_all_prime_ids())
            ->setParameter('prime_member', [1,2])
            ->addGroupBy('ro.Customer')
        ;
    ...
    public function getPrimeContinueProducts($customer_id){
        ...
        $qb
            ...
            //)->andWhere('c.Customer', '=', ':customer_id')
                ...
                $qb->expr()->eq('ro.Customer', ':customer_id')
            )
            //->andWhere(
            //    $qb->expr()->gte('rs.next_delivery_date', ':deadLineDateStart')
            //)
            ...
            //)
            ->andWhere($qb->expr()->in('roi.Product',':products'))
            ->setParameter('regularStatusIds', [RegularStatus::CONTINUE])
        //    ->setParameter('deadLineDateStart', $deliveryNoticeStartDate)
        //    ->setParameter('deadLineDateEnd', $deliveryNoticeEndDate)
            ->setParameter('products', Product::get_all_prime_ids())
            ->setParameter('prime_member', [1,2])
