...
use Customize\Service\WordpressCommand;
...
class PaymentNotificationController extends AbstractController
{
    ...
    public function receiveComplete(Request $request): Response
    {
        ...
        if (!$Order) {
            ...
            logs('gmo_epsilon')->addInfo('決済完了通知 : end. 対象データ処理済み orderId=' . $orderId);
            ...
            logs('gmo_epsilon')->addInfo('決済完了通知 : end after save_family ');
    ...
    private function pt_bak($Customer){

       if($Customer->getPreOrderDiscountPrice() > 0){
         $price = $Customer->getPreOrderDiscountPrice();//$_SESSION["pre_order_discount_price"]["price"];
         $point = $Customer->getPoint() + $price; 
         $Customer->setPoint($point);
         $Customer->setPreOrderDiscountPrice($price * -1);
       }
       log_info("[ポイントバック] pt_bak", [ $Customer->getPreOrderDiscountPrice() ]);
        
    }


    private function save_family(){
        logs('gmo_epsilon')->info('受注ID: '.$this->Order->getId().' family check.');
        

        // OK
        if (!is_null($this->Order->getPayment()) ){
            logs('gmo_epsilon')->info('受注ID: '.$this->Order->getId().' order check.');
        
            $OrderItems = $this->Order->getItems();
            $Config = $this->configRepository->find(1);

            $flg = false;
            $prime_light_flg = false;
            foreach($OrderItems as $item){
                if($item && $item->getProduct()){
                    if($item->getProduct()->getId() == $Config->getPrimeProductId()) {
                        $flg = true;
                    }
                    if($item->getProduct()->getId() == $Config->getPrimeLightProductId()) {
                        $prime_light_flg = true;
                    }
                }
            }
            logs('gmo_epsilon')->info('定期　受注ID: '.$this->Order->getId().' prime_check : ' . $flg . "  prime_light_check : " . $prime_light_flg);
            if($prime_light_flg){
                $Customer = $this->Order->getCustomer();
                if($Customer->getPrimeMember() == 0){
                    $Customer->setPrimeMember(2);

                    $this->pt_bak($Customer);
            ...
            if($flg){
                ...
                logs('gmo_epsilon')->info('受注ID: '.$this->Order->getId().' customer prime check : ' . $Customer->getPrimeMember());
                if($Customer->getPrimeMember() != 1){
                    $Customer->setPrimeMember(1);
                    $this->entityManager->persist($Customer);

                    logs('gmo_epsilon')->info('受注ID: '.$this->Order->getId().' entityManager OK');
                    $ret = WordpressCommand::create_user($this->Order->getId(), $Customer["id"], $Customer["email"], $Customer["name01"], $Customer["name02"]);
                    logs('gmo_epsilon')->info('Wordpress: '. print_r($ret, true));
                    
                    if($ret && isset($ret["password"])){
                        $body = [];
                        $body[] = "";
                        $body[] = sprintf('ログインID: %s', $ret["id"]);
                        $body[] = "";
                        $body[] = sprintf('%s', $ret["password"]);
                        $body[] = "";
                        $body[] = "https://kyogokupro.com/academy/login/";
                        $body[] = "";
                        $body = implode("\n", $body);

                        $MailHistory = new MailHistory();
                        $MailHistory->setMailSubject("[京極アカデミー] ご登録ありがとうございます");
