...
class RegularProductQuantityController extends AbstractController
{
    ...
    public function index(RegularOrder $RegularOrder, Request $request)
    {
        ...
        $form = $this->createForm(RegularProductQuantityType::class, $RegularOrder->getRegularOrderItems()->first());
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            /** @var RegularOrderItem $RegularOrderItem */
            $RegularOrderItem = $RegularOrder->getRegularOrderItems()->first();
            $subTotal = $RegularOrderItem->getPriceIncTax() * $form['quantity']->getData();
            $total = $RegularOrder->getCharge() +
        ...
        return [
            ...
            'Product' => $RegularOrder->getRegularProductOrderItems()[0]->getProductClass()->getProduct(),
        ];
    }
}
