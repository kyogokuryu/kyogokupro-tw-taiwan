...
class RegularCancelController extends AbstractController
{
    ...
    public function cancel(Request $request, RegularOrder $RegularOrder)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            foreach( $RegularOrder->getItems() as $k=>$v){
                $p = $v->getProduct();
                if($p){
                    $Config = $this->configRepository->find(1);
                    if($p->getId() == $Config->getPrimeProductId()){
                        $prime_flg = true;
                    }
                    if($p->getId() == $Config->getPrimeLightProductId()){
                        $prime_light_flg = true;
                    }
                }
            }
/*
  解約申請後、期限まで会員種別の変更はしない
            if($prime_light_flg){
                $Customer = $this->getUser();
                if($Customer->getPrimeMember() == 2){
...
*/
            if($prime_flg){
                ...
                */
                    // wp
                    WordpressCommand::cancel($Customer["id"]);
/*
                    $param = [];
                    $param[] = "user update ";
                    $param[] = "eu" . $Customer["id"];
                    ...
                    $param[] = sprintf('where ID in (select ID from wp_users where user_login="%s")' . "'", "eu" . $Customer["id"]);
                    $param[] = "--path=".__DIR__ . "/../../../../../../academy";
                    ...
                    $param[] = sprintf('(select ID from wp_users where user_login="%s"),', "eu" . $Customer["id"]);
                    $param[] = '"eu' . $Customer["id"] . '",';
                    $param[] = sprintf('(select user_email from wp_users where user_login="%s"),', "eu" . $Customer["id"]);
                    $param[] = sprintf('"%s",', date('Y-m-d'));//start_under25_date="' . date('Y-m-d') . '", nextClass="under-25" ';
                    $param[] = sprintf('"%s")', date('Y-m-d', strtotime('+1 month'))) . "'";//where ID in (select ID from wp_users where user_login="%s")' . "'", "eu" . $Customer["id"]);
                    $param[] = "--path=".__DIR__ . "/../../../../../../academy";
