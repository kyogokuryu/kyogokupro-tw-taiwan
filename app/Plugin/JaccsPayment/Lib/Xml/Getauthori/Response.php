...
use Plugin\JaccsPayment\Lib\Xml\Getauthori\Request\TransactionInfo;
    ...
    public function __construct($xml = null)
    {
        ...
        if ($xml) {
            ...
            foreach ($tags as $tag) {
                ...
                $this->getTransactionInfo()->setManualAuthoriresult($tag->getElementsByTagName('manualAuthoriresult')->item(0)->nodeValue);
                $this->getTransactionInfo()->setManualAuthorireasons(new TransactionInfo\ManualAuthorireasons());
                $ta = $tag->getElementsByTagName('manualAuthorireasons');
                foreach ($ta as $a) {
                    foreach ($a->getElementsByTagName('manualAuthorireason') as $key => $value) {
                        $this->getTransactionInfo()->getManualAuthorireasons()->addManualAuthorireason($value->nodeValue);
                    }
                    break;
                }
                break;
