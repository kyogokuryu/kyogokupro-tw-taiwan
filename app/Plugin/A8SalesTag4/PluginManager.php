...
class PluginManager extends AbstractPluginManager
{
     ...
     * @var string ブロック名
     */
    private $blockName = 'a8 sales tag 4';

    /**
     * @var string ブロックファイル名
     */
    private $blockFileName = 'a8_sales_tag_4_plugin_block';
    ...
    private function createDataBlock(ContainerInterface $container)
    {
	    ...
	    $this->registBlock(Layout::DEFAULT_LAYOUT_UNDERLAYER_PAGE, $Block, $em);
	    $this->registBlock(Layout::DEFAULT_LAYOUT_TOP_PAGE, $Block, $em);
        } catch (\Exception $e) {
            throw $e;
        }
    }

    private function registBlock($layout, $Block, $em) {
            // BlockPositionの登録
            $blockPos = $em->getRepository(BlockPosition::class)->findOneBy(
                ['section' => Layout::TARGET_ID_HEAD, 'layout_id' => $layout],
                ['block_row' => 'DESC']
            );
            ...
            $LayoutDefault = $em->getRepository(Layout::class)->find($layout);

            $BlockPosition->setLayout($LayoutDefault)
                ->setLayoutId($LayoutDefault->getId())
                ->setSection(Layout::TARGET_ID_HEAD)
                ->setBlock($Block)
