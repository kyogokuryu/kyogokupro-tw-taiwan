...
class MailMagazineController extends AbstractController
{
    ...
    public function select(Request $request, $id = null)
    {
        ...
        if ($request->get('mode') == 'select') {
            ...
            if ($id) {
                ...
                $form->get('send_schedule')->setData(date('Y-m-d H:i'));
            ...
            if ($form->isValid()) {
                return $this->render('@MailMagazine4/admin/confirm.twig', [
                    ...
                    'schedule_itm' => date_format($form['send_schedule']->getData(), 'Y-m-d H:i'),
    ...
    public function prepare(Request $request)
    {
        ...
        /** @var \Datetime */
        $sendSchedule = $data['send_schedule'];
        $timezone     = date_default_timezone_get();
        $sendSchedule = $sendSchedule->setTimezone(new DateTimeZone($timezone))->format('Y-m-d H:i');
        $now          = (new \DateTime('now', new DateTimeZone($timezone)))->format('Y-m-d H:i');

        // send schedule is now
        if ($sendSchedule <= $now) {
    ...
    public function sendTest(Request $request)
    {
        ...
        $sent = $this->mailMagazineService->sendTestMail($data);

        log_info('テストメール配信処理完了');

        return $this->json(['status' => true, 'sent' => $sent]);
    }
}
