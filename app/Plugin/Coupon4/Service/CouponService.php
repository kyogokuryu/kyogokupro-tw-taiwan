...
class CouponService
{
    ...
    public function saveCouponOrder(Order $Order, Coupon $Coupon, $couponCd, Customer $Customer, $discount)
    {
        ...
        // 配送割引の確認
        $CouponOrder->setDeliveryFreeFlag(true);
        $CouponDetails = $Coupon->getCouponDetails();
        foreach ($Order->getItems()->getProductClasses() as $orderItem) {
            foreach ($CouponDetails as $CouponDetail) {
                if ($CouponDetail->getProduct()) {
                    //--------------------
                    // 商品
                    //--------------------
                    if ($orderItem->getProduct()->getId() == $CouponDetail->getProduct()->getId()) {
                        if ($CouponDetail->getDeliveryFreeFlag() == 1) {
                            $CouponOrder->setDeliveryFreeFlag(false);
                            break;
                        }
                    }
                    ...
                    // カテゴリ
                    //--------------------
                    $Categories = $orderItem->getProduct()->getProductCategories();
     ...
     * @param Order  $Order default(null)
     * @return float|int|string
     */
    public function recalcOrder(Coupon $Coupon, $couponProducts, Order $Order = NULL)
    {
        $discount = 0;
        // クーポンコードが存在する場合カートに入っている商品の値引き額を設定する
        if ($Coupon) {

// log_info('******** strart racalecOrder');

            $couponDiscountPrice = $Coupon->getDiscountPrice();
            $couponDiscountRate = $Coupon->getDiscountRate();
            $CouponDetails = $Coupon->getCouponDetails();
            // $couponDeliveryFree = 0;
            // $deliveryLoopEnd = false;
            // $orderDelivery = 0;
//             if (!is_null($Order)){
//                 $orderDelivery = $Order->getDeliveryFeeTotal();
// // log_info('******** orderDelivery -> '.$orderDelivery);
//             }
            ...
            if ($Coupon->getDiscountType() == Coupon::DISCOUNT_PRICE) {
                ...
                //     foreach ($CouponDetails as $CouponDetail) {
                //         if ($value['product_id'] == $CouponDetail->getProduct()->getId()) {
                //             if ($CouponDetail->getDeliveryFreeFlag() == 1) {
                //                 // 配送無料の場合は、配送料分を割引
                //                 $discount += $orderDelivery;
                //                 $deliveryLoopEnd = true;
                //                 break;
                //             }
                //         }
                //     }
                //     if ($deliveryLoopEnd === true) {
                //         break;
                //     }
                // }
            } else {
                    ...
                    foreach ($CouponDetails as $CouponDetail) {
                        if ($CouponDetail->getProduct()) {
                            ...
                            if ($value['product_id'] == $CouponDetail->getProduct()->getId()) {
                                ...
                                // if ($CouponDetail->getDeliveryFreeFlag() == 1) {
                                //     $couponDeliveryFree = $orderDelivery;
                                // }
                            }
                        } else if($CouponDetail->getCategory()) {
                            //--------------------
                            // カテゴリ
                            //--------------------
                            $Categories = $value['product']->getProductCategories();
                            foreach ($Categories as $Category) {
                                if ($Category->getCategoryId() == $CouponDetail->getCategory()->getId()) {
                                    if ($CouponDetail->getDetailDiscountRate() != '' && $CouponDetail->getDetailDiscountRate() != 0) {
                                        $couponDiscountRate = $CouponDetail->getDetailDiscountRate();
