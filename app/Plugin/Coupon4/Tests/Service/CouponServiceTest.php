...
class CouponServiceTest extends EccubeTestCase
{
    ...
    public function testSetOrderCompleteMailMessage()
    {
        ...
        $this->assertRegExp('/プロモーションコード情報/u', $Order->getCompleteMailMessage());
        $this->assertRegExp('/プロモーションコード: '.$couponCd.' '.$couponName.'/u', $Order->getCompleteMailMessage());

        $this->couponService->setOrderCompleteMailMessage($Order, null, null);
        $this->assertNotRegExp('/プロモーションコード情報/u', $Order->getCompleteMailMessage());
        $this->assertNotRegExp('/プロモーションコード: '.$couponCd.' '.$couponName.'/u', $Order->getCompleteMailMessage());
    }
    ...
    public function testRemoveCouponOrder()
    {
        ...
        $this->assertTrue($CouponOrderItems->isEmpty(), 'プロモーションコード明細が削除されている');
    ...
    private function getTestData($couponType = Coupon::PRODUCT, $discountType = Coupon::DISCOUNT_PRICE)
    {
        ...
        $Coupon->setCouponName('プロモーションコード');
