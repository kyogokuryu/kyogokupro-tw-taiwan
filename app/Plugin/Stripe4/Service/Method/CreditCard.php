...
class CreditCard implements PaymentMethodInterface
{
    ...
    {
        // pm_idバリデーション
        if (null === $this->Order->getStripePaymentMethodId() || '' === $this->Order->getStripePaymentMethodId()) {
            $result = new PaymentResult();
            $result->setSuccess(false);
            $result->setErrors([trans('請輸入您的信用卡號、有效期和安全碼 (CVV) 信息。')]);
            return $result;
        }
    ...
    public function apply()
    {
        ...
        if (null === $this->Order->getStripePaymentMethodId() || '' === $this->Order->getStripePaymentMethodId()) {
            ...
            $dispatcher->setRoute('shopping_error');
            return $dispatcher;
        }
