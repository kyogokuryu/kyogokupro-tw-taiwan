    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder
            ->addEventListener(FormEvents::POST_SET_DATA, function (FormEvent $event) {
                ...
                if ($order->getPayment() && $order->getPayment()->getMethodClass() === CreditCard::class) {
                    $form
                    ...
                    if ($customer = $order->getCustomer()) {
                        ...
                        if ($customer->hasStripeCustomerId()) {
                            logs('stripe')->info('Stripe 客户 ID 密钥' , [$customer->getStripeCustomerId()]);
                            try{
                                $cards = PaymentMethod::all([
                                    'customer' => $customer->getStripeCustomerId(),
                                    'type' => 'card',
                                ]);
                                logs('stripe')->info('Stripe PaymentMethod' , [$cards]);
        ...
        $builder
            ->addEventListener(FormEvents::SUBMIT, function (FormEvent $event) {
                ...
                if (is_object($order) && $order->getPayment() && $order->getPayment()->getMethodClass() === CreditCard::class) {
                    if ($form->get('redirect_to')->getData()) {
                        return;
                    }

                    if ($form->has('stripe_payment_method_id')) {
                        logs('stripe')->info('输入确认错误', [$form->get('stripe_payment_method_id')]);

                        if (null === $order->getStripePaymentMethodId()) {
                            $form->get('stripe_payment_method_id')->addError(new FormError(trans('请输入您的信用卡号、有效期和 3 位数安全码 (CVV) 信息。')));
                        }
                    }
                }
            });
