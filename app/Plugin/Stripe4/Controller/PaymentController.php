...
class PaymentController extends AbstractShoppingController
{
    ...
    public function payment(): RedirectResponse
    {
        ...
        try {
            if (null !== $paymentMethodId) {
                ...
                $paymentIntentData = [
                    'amount' => $Order->getPaymentTotal() * 100,
                    'currency' => $this->eccubeConfig['currency'],
                    'payment_method' => $paymentMethodId,
                    'confirmation_method' => 'manual',
                    'confirm' => true,
                    'capture_method' => $this->config->getCapture() ? 'automatic' : 'manual',
                    'return_url' => $this->generateUrl('shopping_complete', [], UrlGeneratorInterface::ABSOLUTE_URL),
                ];
                logs('stripe')->info('paymentIntentData', [$paymentIntentData]);
                ...
                if($paymentIntentData["amount"] > 0){
                    ...
                    logs('stripe')->info('0 amount Payment using all points', [$Order->getId()]);
                    $this->entityManager->flush();
                    return $this->nonResponse($Order);
                }
    ...
    protected function nonResponse(Order $Order): RedirectResponse
    {
        logs('stripe')->info("no Response" ,[$Order->getId()]);
