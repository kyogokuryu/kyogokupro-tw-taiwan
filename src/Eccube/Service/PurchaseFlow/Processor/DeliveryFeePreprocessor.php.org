...
class DeliveryFeePreprocessor implements ItemHolderPreprocessor
{
    ...
    private function saveDeliveryFeeItem(ItemHolderInterface $itemHolder)
    {
        ...
        foreach ($Order->getShippings() as $Shipping) {
            ...
            // カスタマイズ
            $price = $DeliveryFee->getFee() + $deliveryFeeProduct;
            $CouponOrders = $this->entityManager->getRepository('Plugin\Coupon4\Entity\CouponOrder')->findBy(['order_id' => $Order->getId()]);
            foreach ($CouponOrders as $couponorder) {
                // log_info('couponorder->'.$couponorder->getCouponName());
                if ($couponorder->getDeliveryFreeFlag() == 0) {
                    $price = 0;
                }
            }
            // カスタマイズ
