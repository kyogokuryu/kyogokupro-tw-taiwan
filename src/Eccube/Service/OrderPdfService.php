...
class OrderPdfService extends TcpdfFpdi
{
    ...
    public function __construct(EccubeConfig $eccubeConfig, OrderRepository $orderRepository, ShippingRepository $shippingRepository, TaxRuleService $taxRuleService, BaseInfoRepository $baseInfoRepository, EccubeExtension $eccubeExtension, TaxExtension $taxExtension)
    {
        ...
        $this->labelCell[] = '数量';
        $this->labelCell[] = '単価';
        $this->labelCell[] = '金額(含稅)';
        $this->widthCell = [105.3, 12, 25.7, 24.5];
    ...
    {
        // 発行日の設定
        $this->issueDate = '印單日期: '.$formData['issue_date']->format('Y年m月d日');
        // ダウンロードファイル名の初期化
        ...
        foreach ($ids as $id) {
            ...
            if ($Order->isMultiple()) {
                ...
                $userPath = $this->eccubeConfig->get('eccube_html_admin_dir').'/assets/pdf/nouhinsyo2.pdf';
            }
    ...
    protected function renderShopData()
    {
        ...
        $logoFile = $this->eccubeConfig->get('eccube_html_admin_dir') . '/assets/img/taiwan_logo.png';
        $this->Image($logoFile, 83, 38 + $top, 40);

        // $this->lfText(85, 75 + $top, $this->baseInfoRepository->getCompanyName(), 14, 'B');
        // $this->lfText(125, 60, $this->baseInfoRepository->getShopName(), 12, 'B');

        // 都道府県+所在地
        // $text = $this->baseInfoRepository->getAddr01();
        $this->lfText(77, 82 + $top, '果然有魚國際行銷有限公司', 12);
        $this->lfText(60, 88 + $top, '台中市南屯區文心路一段218號19樓之3', 12);

        // 電話番号
        $text = 'TEL: '.$this->baseInfoRepository->getPhoneNumber();
        $this->lfText(83, 94 + $top, $text, 12);

        // メールアドレス
        if (strlen($this->baseInfoRepository->getEmail01()) > 0) {
            $text = 'Email: '.$this->baseInfoRepository->getEmail01();
            $this->lfText(65, 100 + $top, $text, 12);
        }
        ...
        //     $logoFile = $this->eccubeConfig->get('eccube_html_admin_dir').'/assets/img/kyogoku_logo.png';
        // }

        // $this->Image($logoFile, 126, 40 + $top, 20);
    }
    ...
    {
        $top = $this->top;
        $this->lfText(19, 70 + $top, $formData['message1'], 10); //メッセージ1
        $this->lfText(19, 74 + $top, $formData['message2'], 10); //メッセージ2
        // $this->lfText(19, 78 + $top, $formData['message3'], 8); //メッセージ3
    }
...
//        $this->Cell(0, 5, "\n", 0, 1, 'L', 0, '');
//        $this->lfText(0, 0, $formData['note1'], 0, 1, 'L', 0, '');
//        $this->lfText(0, 0, $formData['note2'], 0, 1, 'L', 0, '');
//        $this->lfText(0, 0, $formData['note3'], 0, 1, 'L', 0, '');
        $this->lfText(20, 100 + $top, '＜ 備考 ＞', 9);
        $this->lfText(20, 110 + $top, $formData['note1'], 8);
        $this->lfText(20, 115 + $top, $formData['note2'], 8);
        $this->lfText(20, 120 + $top, $formData['note3'], 8);
    ...
    protected function renderTitle($title)
    {
        ...
        if (strpos($title, '果然有魚國際行銷有限公') !== false) {
            $this->line(20, 10, 200, 10);
            $this->line(20, 25, 200, 25);
        } else {
            $this->line(20, 10, 200, 10);
            $this->line(20, 20, 200, 20);
        }
    ...
    protected function renderOrderData(Shipping $Shipping)
    {
        ...
        // $textAddr01 = $Shipping->getPref().$Shipping->getAddr01();
        // $textAddr02 = $Shipping->getAddr02(); //購入者住所2
        // $this->lfText(19, 47 + $top, $textAddr01 . ' ' . $textAddr02, 10);

        // 購入者氏名
        // $text = $Shipping->getName01().' '.$Shipping->getName02();
        // $this->lfText(19, 55 + $top, $text, 11);
        ...
        if (strpos($this->title, '出貨單') !== false) {
            $this->SetFont(self::FONT_SJIS, 'B', 12);
            $this->lfText(70, 108 + $top, "【購買日期】" . $orderDate, 11);
            $this->lfText(70, 114 + $top, "【訂單編號】". $Order->getOrderNo(), 11);
        } else {
            $this->SetFont(self::FONT_SJIS, 'B', 12);
            $this->lfText(70, 108 + $top, "【購買日期】" . $orderDate, 11);
            //注文番号
            $this->lfText(70, 114 + $top, "【訂單編號】". $Order->getOrderNo(), 11);
            $this->lfText(70, 120 + $top, "【發票日期】" . $Order->getInvoiceDate()->format('Y/m/d H:i'), 11);
            $this->lfText(70, 126 + $top, "【發票號碼】" . $Order->getInvoiceNumber(), 11);
        }
    ...
    protected function renderOrderDetailData(Shipping $Shipping)
    {
        ...
        foreach ($Shipping->getOrderItems() as $OrderItem) {
            ...
            // product
            //セット商品の場合は個数を表示 20211126 kikuzawa
            $selectedItems = [];
            if($OrderItem['ksTmpProductName']){
                $productName = $OrderItem['ksTmpProductName'];
                foreach($OrderItem['KsOrderItemChildren'] as $ksG){
                    $groupId = $ksG['ksSelectItemGroupId'];
                    foreach ($OrderItem['Order']['OrderItems'] as $item) {

                        if($item["id"] == $ksG["OrderItem"]["id"]){
                            $code = $item["product_code"];
                            if(!isset($selectedItems[$code])){
                                $selectedItems[$code] = ["product_name"=>$item["product_name"], "product_code"=>$item["product_code"], "quantity"=>0];
                            }
                            $selectedItems[$code]["quantity"] += $item["quantity"];                        
                        }
                        /*
                        if($item['product_code'] && !strpos($item['product_name'], 'セット') && strpos($item['product_name'], '選択商品') && $item['KsOrderItemEx']['ksSelectItemGroupId'] == $groupId){
                            //$productName .= "\n".$item['product_name'].'('.$item['quantity'].'個)';
                            $code = $item["product_code"];
                            if(!isset($selectedItems[$code])){
                                $selectedItems[$code] = ["product_name"=>$item["product_name"], "product_code"=>$item["product_code"], "quantity"=>0];
                            }
                ...
                if($p && $p->getProductSubName()){
                    $productName = $p->getProductSubName();
                }
                //exit;
                //if($p->getProductSubName()){
                //    $productName = $p->getProductSubName();
                //}
            ...
            // 税込金額（単価）
            $arrOrder[$i][2] = '$' . $OrderItem->getPrice();
            // 小計（商品毎）
            $arrOrder[$i][3] = '$' . $OrderItem->getTotalPrice();


            ++$i;
            // 選択商品[1] KYOGOKU ブロンドカラーシャンプー BL　200ml(1個)
            if(count($selectedItems) > 0){
                foreach($selectedItems as $item){
                    $arrOrder[$i][0] = $item["product_name"] . " (" . $item["product_code"] . ")";
                    $arrOrder[$i][1] = $item["quantity"];
                    $arrOrder[$i][2] = "-";     
                    $arrOrder[$i][3] = "-";
                    ++$i;     
                }
        ...
        if (!$Order->isMultiple()) {
            ...
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '商品小計';
            $arrOrder[$i][3] = '$' . $Order->getSubtotal();

            ++$i;
            $arrOrder[$i][0] = '';
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '運費';
            $arrOrder[$i][3] = '$' . $Order->getDeliveryFeeTotal();

            ++$i;
            $arrOrder[$i][0] = '';
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '手数料';
            $arrOrder[$i][3] = '$' . $Order->getCharge();

            ++$i;
            $arrOrder[$i][0] = '';
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '折扣金';
            $arrOrder[$i][3] = '$' . $Order->getTaxableDiscount();
/*
            ++$i;
            $arrOrder[$i][0] = '';
            $arrOrder[$i][1] = '';
...
*/
            ...
            // $arrOrder[$i][1] = '';
            // $arrOrder[$i][2] = '合計';
            // $arrOrder[$i][3] = '$' . $Order->getTaxableTotal();
            ...
            foreach($Order->getTaxFreeDiscountItems() as $Item) {
                ...
                $arrOrder[$i][1] = '';
                if($p && $p->getProductSubName()){
                    $arrOrder[$i][2] = $p->getProductSubName();
                }else{
                    $arrOrder[$i][2] = $Item->getProductName();
                }
                $arrOrder[$i][3] = '$' . $Item->getTotalPrice();
            }
            ...
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '付款總金額';
            $arrOrder[$i][3] = '$' . $Order->getPaymentTotal();
    ...
    protected function setFancyTable($header, $data, $w)
    {
        ...
        // 開始座標の設定
        $this->setBasePosition(0, 130 + $top);
        ...
        foreach ($data as $row) {
            ...
            $cellHeight = 0;
            foreach ($row as $col) {
                if (mb_strlen($col, 'UTF-8') > 80) {  //商品名が長いときセルの高さを上げる
                    $h = 21;
                }else if (mb_strlen($col, 'UTF-8') > 50) { 
                    $h = 14;
                }
