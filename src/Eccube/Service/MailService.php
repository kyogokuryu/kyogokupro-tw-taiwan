...
class MailService
{
     ...
     * @param $amount 支払額
     */
    public function sendSalonafPaidMail(\Eccube\Entity\Customer $Customer, $month, $sales, $reward)
    {
        log_info('salonaf支払い連絡メール送信開始');

        $month = str_replace('-', '年', $month).'月';
        $sales = '￥'.number_format($sales);
        $reward = '￥'.number_format($reward);
        $name = $Customer['name01'].$Customer['name02'];
        $customer_Account_type = ($Customer['Account_type']) ? $Customer['Account_type'] : 0;
        $Account_type = array(0 => '', 1 => '普通', 2 => '当座');
        $subject = '【重要】Kyogoku Professional アフィリエイト報酬支払い完了のお知らせ]';
        $body = <<<EOD
$name さんアフィリエイト活動にご協力いただきありがとうございます。
支払い手続きが完了しました。
...
【振込先】
金融機関名: {$Customer['Financial']}
支店名: {$Customer['Branch']}
口座種別: {$Account_type[$customer_Account_type]}
口座番号: {$Customer['Account_number']}
口座名義人名(カナ): {$Customer['Account_name']}
...
EOD;
        ...
        log_info('salonaf支払い連絡メール送信完了', ['count' => $count]);
     ...
     *
     * @param $Customer 会員情報
     * @param $addPoint 加算ポイント
     * @param $sender 送り主
     * @param $message メッセージ
     */
    public function sendGiftMail(\Eccube\Entity\Customer $Customer, $addPoint, $sender, $senderMessage)
    {
        log_info('ギフトメール送信開始');

        $name = $Customer['name01'].$Customer['name02'].'様';
        $subject = '【'.$name.'へ ギフトカードが届きました】';
        $body = <<<EOD
$name こんにちは！

$sender より

『ギフトポイント $addPoint 円分』が贈られました。
...
EOD;
        ...
        log_info('ギフトメール送信完了', ['count' => $count]);
