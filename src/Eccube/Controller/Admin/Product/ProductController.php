...
class ProductController extends AbstractController
{
    ...
    public function edit(Request $request, $id = null, RouterInterface $router, CacheUtil $cacheUtil)
    {
        ...
        if (is_null($id)) {
            ...
            $saleQb = $this->orderRepository
                ...
                ->select('SUM(oi.price) AS price');
            $result = $saleQb->getQuery()->getSingleResult();
            $price = $result['price'];
        }
        ...
        foreach ($ProductImages as $ProductImage) {
            if ($ProductImage->getCategory() === 0) { // categoryが0のものだけを取得
                $images[] = $ProductImage->getFileName();
            }else{
                $descriptionImages[] = $ProductImage->getFileName();
            }
        }
        $form['images']->setData($images);
        $form['description_images']->setData($descriptionImages);
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                //ギフトカードのカテゴリーに属する場合はポイント付与の対象から除外 20220822 kikuzawa
                if(!$has_class && array_key_exists(39, $categoriesIdList)){
                    $ProductClass->setPointRate(0);
                }
                ...
                // 説明用画像の登録
                $add_images = $form->get('description_add_images')->getData();
                foreach ($add_images as $add_image) {
                    $ProductImage = new \Eccube\Entity\ProductImage();
                    $ProductImage
                        ->setFileName($add_image)
                ...
                // 説明用画像の削除
                $delete_images = $form->get('description_delete_images')->getData();
                foreach ($delete_images as $delete_image) {
                    $ProductImage = $this->productImageRepository
                        ->findOneBy(['file_name' => $delete_image]);
                ...
                $brandId = $form->get('Brand')->getData();
                if ($brandId) {
     ...
     * @param Request $request
     *
     * @Route("/%eccube_admin_route%/product/sale", name="admin_product_sale")
     *
    ...
    public function sale(Request $request){
        ...
        for ($i=0; $i<13; $i++) {
            $dateTime = (new \DateTime('-'. (string)$i .' month'))->modify('first day of this months')->setTime(0, 0, 0, 0);
            ...
            $saleQb = $this->orderRepository
                ...
                ->select('SUM(oi.price * oi.quantity) AS sale');
            $result = $saleQb->getQuery()->getSingleResult();
            $sale = $result['sale'] ?? 0;
            array_push($saleForEachMonth, ["sale"=> $sale, "date" => $dateTime->format('Y/m')]);
        }
        $saleForEachMonth = array_reverse($saleForEachMonth);

        // 直近1週間の毎日の売上
        $weeklySales = $this->calculateWeeklySales($id);

        // 今月の毎日の売上
        $monthlySales = $this->calculateMonthlySales($id);

        // 直近10年間の年間売上
        $yearlySales = $this->calculateYearlySales($id);

        return $this->json([$weeklySales, $monthlySales, $saleForEachMonth, $yearlySales]);
    }

    private function calculateDailySale($productId, $start, $end) {
        $saleQb = $this->orderRepository
            ->createQueryBuilder('o')
            ->join('o.OrderItems', 'oi')
            ->join('oi.Product', 'p')
            ...
            ->setParameter(':targetDateStart', $start)
            ->setParameter(':targetDateEnd', $end)
            ->andWhere(':targetDateStart <= o.order_date and o.order_date < :targetDateEnd')
            ->select('SUM(oi.price * oi.quantity) AS sale');
    
        $result = $saleQb->getQuery()->getSingleResult();
        return $result['sale'] ?? 0;
    }

    private function calculateWeeklySales($id) {
        $endDate = (new \DateTime())->modify('+1 day');
        $startDate = (clone $endDate)->modify('-6 days');

        $weeklySalesData = [];
        while ($startDate <= $endDate) {
            $dateTimeStart = clone $startDate;
            // $dateTimeStart->setTime(0, 0, 0, 0);
            $dateTimeEnd = clone $startDate;
            $dateTimeEnd->modify('+1 day');

            $dailySale = $this->calculateDailySale($id, $dateTimeStart, $dateTimeEnd);
            $weeklySalesData[] = ["sale" => $dailySale, "date" => $dateTimeStart->format('Y/m/d')];
    ...
    private function calculateMonthlySales($id) {
        $monthlySalesData = [];
        ...
        while ($startDate <= $endDate) {
            ...
            $dailySale = $this->calculateDailySale($id, $dateTimeStart, $dateTimeEnd);
            $monthlySalesData[] = ["sale" => $dailySale, "date" => $dateTimeStart->format('Y/m/d')];
    ...
    private function calculateLast12MonthsSales($productId)
    {
        $last12MonthsSales = [];
        ...
        for ($i = 0; $i < 12; $i++) {
            ...
            // 月の売上を計算
            $monthlySale = $this->calculateMonthlySale($productId, $startDate, $endDate);
        ...
        return $last12MonthsSales;
    }
    
    private function calculateYearlySales($id) {
        $yearlySalesData = [];

        // 直近10年間の年を計算
        $currentYear = new \DateTime('now');
        $startYear = (clone $currentYear)->modify('-9 years');

        while ($startYear <= $currentYear) {
            $dateTimeStart = clone $startYear;
            $dateTimeStart->modify('first day of January');//->setTime(0, 0, 0, 0);
            $dateTimeEnd = clone $startYear;
            $dateTimeEnd->modify('first day of January')->modify('+1 year');

            $yearlySale = $this->calculateYearlySale($id, $dateTimeStart, $dateTimeEnd);
            $yearlySalesData[] = ["sale" => $yearlySale, "date" => $dateTimeStart->format('Y')];

            $startYear->modify('+1 year');
        }
        return $yearlySalesData;
    }

    private function calculateYearlySale($productId, $start, $end) {
        $saleQb = $this->orderRepository
            ->createQueryBuilder('o')
            ->join('o.OrderItems', 'oi')
            ->join('oi.Product', 'p')
